...
- `index3.html` carga este catalogo via `fetch` async al iniciar, lo matchea por tokens contra el modelo (reusando el indice `liveAliasIndex`) y adjunta `feature.cat`. Las paginas index/index2 NO cargan catalogo (A/B limpio). El parque cambia poco; se re-importa manualmente cuando el CEN publique reportes actualizados. No hay simulacion: sin match -> `En Desarrollo`.
