...
- Fechas disponibles reales por seccion (agosto 2026): Generacion por Tecnologia 2026-01 a 2026-08 (mensual); Generacion por Nodo 2026-01-01 a 2026-08-12 (horaria, junio cortado al 18 y agosto al 12); Transmision/Flujos solo 2026-06-19 a 2026-06-25 (una semana); CMg la ventana que el cron de `update-cen-data.yml` publique (p.ej. 2026-08-07 a 2026-08-12, o los 3 meses de retencion del backfill manual). No existe un dia comun entre secciones.
- Estado CMg: disponible desde el workflow programado `update-cen-data.yml` (cron cada 15 min, usa `CEN_API_KEY`/`CEN_OPERACION_USER_KEY` de GitHub Secrets). Ya genera `docs/data/live/cmg-current.json` (ok, barras reales, ventana ~7 dias, `stale: true` cuando la API no actualiza) y `docs/data/history/cmg/YYYY-MM.json`. La retencion del backfill es de 3 meses (CMg definitivo disponible). El backfill manual `backfill-cen-history.yml` (con `CEN_BACKFILL_DATASETS=cmg-real,cmg-online`, hasta `CEN_BACKFILL_MAX_PAGES=50`) se usa para cubrir los meses historicos de retencion; si el commit del runner falla por `non-fast-forward` (porque el cron o un deploy adelanto `main`), hay que re-disparar. Corridas locales sin la variable `CEN_API_KEY` devuelven `403 Forbidden Authentication parameters missing`.
